#!/usr/bin/env python3
# Copied from original and modified, e.g. to use random tiebreaks.
...
sys.path.insert(1, "/gpfs/home/dmunro/tools/tensorqtl/tensorqtl")
# from core import *
# from post import *
# import genotypeio
# import cis
# import trans
import tensorqtl
from tensorqtl import genotypeio, cis, SimpleLogger, read_phenotype_bed
...
def main():
    ...
    device = "cpu" # I'm getting torch import errors -DM
    ...
    if args.mode.startswith('cis'):
        if args.mode=='cis':
            res_df = cis.map_cis(genotype_df, variant_df, phenotype_df, phenotype_pos_df, covariates_df,
                                 ...
                                 random_tiebreak=True) # Added -DM
            logger.write('  * writing output')
            if tensorqtl.has_rpy2:
                tensorqtl.calculate_qvalues(res_df, fdr=args.fdr, qvalue_lambda=args.qvalue_lambda, logger=logger)
