...
p <- add_argument(p, "--cores", default=8, help="Number of cores (0 for all)")
...
CIS_WINDOW <- 1e6
# N_CORES <- 8  # 0 to use all available cores
...
scan_gene <- function(gene_id) {
    ...
    if (nrow(in_window) > 0) {
        ...
        out <- scan1(pr_gene, pheno_gene, kinship[loc$chr], cores = argv$cores)
        out %>%
...
scan_gene_perm <- function(gene_id) {
    ...
    if (nrow(in_window) > 0) {
        ...
        pr_gene[[loc$chr]] <- pr_gene[[loc$chr]][, , in_window$id, drop = FALSE]
        pheno_gene <- cross$pheno[, gene_id, drop = FALSE]
        out <- scan1perm(pr_gene, pheno_gene, kinship[loc$chr], n_perm = argv$n_perms,
                         cores = argv$cores)
...
scan_gene_coef <- function(gene_id) {
    ...
    if (nrow(in_window) > 0) {
        ...
        pheno_gene <- cross$pheno[, gene_id, drop = FALSE]
        out <- scan1coef(apr_gene, pheno_gene, kinship[[loc$chr]], cores = argv$cores)
        out %>%
            as_tibble(rownames = "snp") %>%
...
pr <- calc_genoprob(cross, error_prob = argv$error_prob, cores = argv$cores)
kinship <- calc_kinship(pr, "loco", cores = argv$cores)
