...
def genotype_code(gt: tuple, founder: bool = False) -> str:
    if gt == (None, None):
        return "-" if founder else ("-", "-")
    elif gt == (0, 0):
        return "A" if founder else ("A", "A")
    elif (gt[0] == 0 and gt[1] > 0):
        return "-" if founder else ("A", "B")
    elif (gt[0] > 0 and gt[1] == 0):
        return "-" if founder else ("B", "A")
    elif gt[0] > 0 and gt[1] > 0:
        return "B" if founder else ("B", "B")
    else:
...
SNPS = sys.argv[3]
GMAP_DIR = sys.argv[4]
GENO1_OUT = sys.argv[5]
GENO2_OUT = sys.argv[6]
FOUNDER_OUT = sys.argv[7]
PMAP_OUT = sys.argv[8]
GMAP_OUT = sys.argv[9]
...
for rec in vcf.fetch():
    ...
    if IDs is None or ID in IDs:
        ...
        labels = list(zip(*[genotype_code(g, founder=False) for g in gt]))
        genos1[ID] = labels[0]
        genos2[ID] = labels[1]
        refs[ID] = rec.ref

ID_list = [x for x in ID_list if x in genos1.keys()]
IDs = set(ID_list)
...
with open(GENO1_OUT, "w") as out:
    out.write("id,{}\n".format(",".join(samples)))
    for ID in ID_list:
        out.write("{},{}\n".format(ID, ",".join(genos1[ID])))

with open(GENO2_OUT, "w") as out:
    out.write("id,{}\n".format(",".join(samples)))
    for ID in ID_list:
        out.write("{},{}\n".format(ID, ",".join(genos2[ID])))
